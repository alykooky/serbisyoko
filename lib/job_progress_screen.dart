...
class MatchingService {
  ...
  /// Ensures we always have numeric scores for total computation
  ...
  // ✅ Added method for JobProgressScreen
  static Future<Map<String, dynamic>> trackJobProgress(String bookingId) async {
    try {
      // Fetch booking
      final booking =
          await _sb.from('bookings').select('*').eq('id', bookingId).single();

      // Fetch related job progress (optional table)
      ...
      return {
        'booking': booking,
        'client_progress': progress?['client_progress'] ?? '',
        'worker_progress': progress?['worker_progress'] ?? '',
        'next_steps': progress?['next_steps'] ?? [],
      };
    } catch (e) {
      debugPrint('❌ trackJobProgress failed: $e');
      rethrow;
    }
  }
}
